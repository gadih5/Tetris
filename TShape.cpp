...
TShape::TShape(const Point& start_point) :
	m_rotation_status(RotationStatus::FirstState)
{
	...
	*/
	...
	Point p4(start_point.get_x() + 1, start_point.get_y() + 1);
...
TShape::TShape(const std::array<Point, 4> points, RotationStatus state) :
...
std::unique_ptr<Shape> TShape::fetch_move(Movement movement)
{
	std::unique_ptr<TShape> temp_shape = std::make_unique<TShape>(m_points, m_rotation_status);
	temp_shape->move(movement);
	return std::move(temp_shape);
}


void TShape::move(Movement movement)
{
...
void TShape::_rotate_clockwise()
{
	...
	case RotationStatus::FirstState:
		...
		m_points[3].set_x(m_points[3].get_x() - 1);
		m_points[3].set_y(m_points[3].get_y() + 1);
...
void TShape::_rotate_counter_clockwise()
{

	switch (m_rotation_status)
	{
	case RotationStatus::RotetedRight:
		// From RotatedRight to FirstState:
		/*
		 *	-#-			---
		 *	##-    <-	###
		 *	-#-			-#-
