#include "LShape.h"
LShape::LShape(const Point& start_point):
...
LShape::LShape(const std::array<Point, 4> points, RotationStatus state) :
...
std::unique_ptr<Shape> LShape::fetch_move(Movement movement)
{
	std::unique_ptr<LShape> temp_shape = std::make_unique<LShape>(m_points, m_rotation_status);
	temp_shape->move(movement);
	return std::move(temp_shape);
}

void LShape::move(Movement movement)
{
...
void LShape::_rotate_counter_clockwise()
{
