...
// Horizonal C'tor
ZShape::ZShape(const Point& start_point, bool is_vertical):
...
ZShape::ZShape(const std::array<Point, 4> points, bool is_vertical):
m_is_vertical(is_vertical)
{
	Point p1(points[0]);
	Point p2(points[1]);
...
std::unique_ptr<Shape> ZShape::fetch_move(Movement movement)
{
	std::unique_ptr<ZShape> temp_shape = std::make_unique<ZShape>(m_points, m_is_vertical);
	temp_shape->move(movement);
	return std::move(temp_shape);
}

void ZShape::move(Movement movement)
{
